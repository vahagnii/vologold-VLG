...
#if defined(NDEBUG)
# error "Vologold cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd484d8df5293f920a2c40e1a94c4dd4cef4fb2ba71c0511177b45c34a0ba17bb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Vologold: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Vologold Signed Message:\n";
...
{
    // Vologold: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 174000 blocks, which will occur approximately every 1 year
    nSubsidy >>= (nHeight / 174000); // Vologold: 174k blocks in ~1 year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.5 * 24 * 60 * 60; // Vologold: 12 hours
static const int64 nTargetSpacing = 3 * 60; // Vologold: 3 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Vologold: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Vologold: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x348a0645afced15e6cffb0e1b85b0935e9b1125e0f4a26a9b20f98ade830f5e5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "New York became the 11th state to ratify the United States Constitution, on July 26, 1788";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046a429642b664934c49e4d6ff41279645f9d252a36423a0e212f5421f1caca121136969c5baf5b94e8d8d03932c2674f65b90343dfec208446255ecae5794e355") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525093901;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086665807;

        if (fTestNet)
        {
            block.nTime    = 1525093884;
            block.nNonce   = 385350222;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x80357f3e784c54e86124632d60013bafd42d0be55c7d31c8ad2a17ae09b645ed"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc4, 0xb7, 0xd4 }; // Vologold: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VologoldMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VologoldMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VologoldMiner(CWallet *pwallet)
{
    printf("VologoldMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vologold-miner");
    ...
    try { loop {
        ...
        printf("Running VologoldMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VologoldMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VologoldMiner, pwallet));
}
